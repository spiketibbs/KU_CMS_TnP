...
def fit_function(
    ...
    if interactive:
        ...
        if not m.valid:
                    print("\n[WARNING] Automated fit failed. Opening interactive plotter for manual adjustment...\n")
                    m.strategy = 1
                    m.interactive(plotter)
    ...
    try:
        m.minos("epsilon") 
        # Uncomment below if you also strictly need MINOS errors for yields
        # m.minos("N")
        # m.minos("B_p")
        # m.minos("B_f")
    except Exception as e:
        logging.debug(f"MINOS failed on epsilon: {e!s}")
