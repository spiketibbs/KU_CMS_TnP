...
# Plot data as density (Events / GeV)
    data_pass_density = results["values_pass"] / bin_widths_pass
    errors_pass_density = results["errors_pass"] / bin_widths_pass
    ax_pass.errorbar(
        results["centers_pass"], data_pass_density, yerr=errors_pass_density, fmt="o", markersize=6, capsize=3, color="royalblue", label="Data (Pass)"
    )
    ...
    info_text = [
        ...
        f"Sig yield: {params['N'] + (params['B_p'] + params['B_f']):.1f}",
...
# Plot data as density (Events / GeV)
    data_fail_density = results["values_fail"] / bin_widths_fail
    errors_fail_density = results["errors_fail"] / bin_widths_fail
    ax_fail.errorbar(
        results["centers_fail"], data_fail_density, yerr=errors_fail_density, fmt="o", markersize=6, capsize=3, color="royalblue", label="Data (Fail)"
    )
    ...
    info_text = [
        ...
        "",
        f"Converged: {results['converged']}",
        f"\nSignal yield: {params['N'] - (params['B_p'] + params['B_f']):.1f}",
