...
def create_combined_model_integral(fit_type, n_pass_edges):
    ...
    def model(xe, *all_pars):
        ...
        # Transform epsilon_raw to epsilon using sigmoid
        epsilon = sigmoid(P["epsilon_raw"]) if "epsilon_raw" in P else P.get("epsilon", 0.9)

        # split raw array
        edges_pass    = xe[:n_pass_edges]
        edges_fail_sh = xe[n_pass_edges:]      
        offset        = edges_pass[-1]
        edges_fail    = edges_fail_sh - offset
...
def fit_function(fit_type, hist_pass, hist_fail, fixed_params=None, x_min=x_min, x_max=x_max):
    ...
    for name in param_names:
        ...
        elif name == "epsilon_raw":
            initial_guesses[name] = 0
        else:
    ...
    offset = edges_pass[-1]
    combined_edges = np.concatenate([edges_pass, edges_fail[1:] + offset])
    ...
    # after fit, build the CDF:
    vals = model_integral(combined_edges, *[popt[n] for n in param_names])
    # then per‑bin expected counts = the differences:
    expected = np.diff(vals)   # length 100
    observed = np.concatenate([values_pass, values_fail])
    ...
    chi2 = 2 * np.sum(expected[mask] - observed[mask] + observed[mask] * np.log(observed[mask] / expected[mask]))
    dof = len(observed) - m.nfit
...
def plot_combined_fit(results, plot_dir=".", data_type="DATA", fixed_params=None):
    ...
    # Convert errors back from sigmoid transformation
    epsilon = sigmoid(params["epsilon_raw"]) if "epsilon_raw" in params else params.get("epsilon", 0.9)
    epsilon_err = sigmoid(results["perr"]["epsilon_raw"]) if "epsilon_raw" in results["perr"] else results["perr"].get("epsilon", 0.9)
    ...
    info_text = [
        ...
        f"ε_raw = {params['epsilon_raw']:.6f} ± {results['perr']['epsilon_raw']:.6f}",
    ...
    info_text = [
        f"N = {params['N']:.1f} ± {results['perr']['N']:.1f}",
        f"ε_raw = {params['epsilon_raw']:.6f} ± {results['perr']['epsilon_raw']:.6f}",
        f"ε = {params['epsilon_raw']:.6f} ± {results['perr']['epsilon_raw']:.6f}",
        f"B_p = {params['B_p']:.1f} ± {results['perr']['B_p']:.1f}",
        f"B_f = {params['B_f']:.1f} ± {results['perr']['B_f']:.1f}",
        f""
...
for sig_name, sig_config in SIGNAL_MODELS.items():
    for bg_name, bg_config in BACKGROUND_MODELS.items():
        ...
        param_names = ["N", "epsilon_raw", "B_p", "B_f"]
...
# NEW FILE PATH
    ...
    epsilon = sigmoid(results['popt']["epsilon_raw"])
    epsilon_err = sigmoid(results["perr"]["epsilon_raw"]) 
    
    root_file.close()
    print(f"\nEfficiency for pt bin {bin_range} GeV = {results['popt']['epsilon_raw']:.4f} ± {results['perr']['epsilon_raw']:.4f}")
    print(f"\nEfficiency after transform for pt bin {bin_range} GeV = {epsilon:.4f} ± {epsilon_err:.4f}")
