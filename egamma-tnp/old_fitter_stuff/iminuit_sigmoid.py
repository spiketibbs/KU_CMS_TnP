...
def create_combined_model_integral(fit_type, n_pass_edges):
    ...
    def model(xe, *all_pars):
        ...
        # Split the combined edges array
        edges_pass = xe[:n_pass_edges]
        edges_fail = xe[n_pass_edges:]
        
        # No offset needed - edges should be in their original ranges
        ...
        # Calculate integrals using original edge ranges
...
def print_fit_progress(m):
    print(f"Iteration: N={m.values['N']:.1f}, ε={m.values['epsilon_raw']:.3f}, "
...
def fit_function(fit_type, hist_pass, hist_fail, fixed_params=None, x_min=x_min, x_max=x_max):
    ...
    initial_epsilon = np.clip(total_pass / (total_pass + total_fail), 0.01, 0.99)
    initial_epsilon_raw = np.log(initial_epsilon / (1 - initial_epsilon))

    # Scale fixed parameters if present
    for name in ['N', 'epsilon_raw', 'B_p', 'B_f']:
    ...
    bounds.update({
        "N":   (0, N_p0*0.7, N_p0*5),
        "B_p": (0, B_p_p0/2, B_p_p0*5),
        "B_f": (0, B_f_p0/2, B_f_p0*5),
        "epsilon_raw": (-10, 0, 10)
    })
    ...
    for name in param_names:
        ...
        if name == "N":
            ...
            initial_guesses[name] = initial_epsilon_raw
        else:
    ...
    m.strategy = 2                 
    m.interactive()
    print(sigmoid(m.values['epsilon_raw']))
    m.fixed['epsilon_raw'] = True
    m.simplex()
    print(sigmoid(m.values['epsilon_raw']))
    m.fixed['epsilon_raw'] = False
    m.migrad()
