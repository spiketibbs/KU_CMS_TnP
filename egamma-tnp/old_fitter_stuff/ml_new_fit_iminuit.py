...
class PassFailPlotter:
    ...
    def __call__(self, args):
        ...
        plt.cla()
        plt.title("Pass")
        plt.errorbar(cx_pass, data_pass, yerr=self.error_pass, fmt='o', color='black', label='Data')  # Use self.error_pass
        ...
        plt.cla()
        plt.title("Fail")
        plt.errorbar(cx_fail, data_fail, yerr=self.error_fail, fmt='o', color='black', label='Data')  # Use self.error_fail
...
def load_histogram(root_file, hist_name, data_label):
    ...
    # Check TH1 histogram type
    try:
        from uproot.behaviors.TH1 import Histogram
        is_hist = isinstance(obj, Histogram)
    except Exception:
        is_hist = hasattr(obj, "to_numpy") and hasattr(obj, "variances")

    if not is_hist:
        print(f"[ERROR] Object '{hist_name}' is not a TH1 histogram.")
        return None
    ...
    if is_mc:
        ...
        mask_var = variances > 0
        if np.any(mask_var):
            errors[mask_var] = np.sqrt(variances[mask_var])
    else:
        try:
            errors = obj.errors()
        except Exception:
...
def linear_pdf(x, b, C, x_min=x_min, x_max=x_max):
    lin = b*x + C
    denom = 0.5*b*(x_max**2 - x_min**2) + C*(x_max - x_min)
...
def linear_cdf(x, b, C, x_min=x_min, x_max=x_max):
    ...
    if den <= 0:
        den = 1e-8
    # vectorized numerator with clipping
...
def create_combined_model(fit_type, edges_pass, edges_fail, *params, use_cdf=False):
    ...
    if use_cdf is True:
        signal_func = config["signal_cdf"]
        bg_func = config["background_cdf"]
    else:
    ...
    # If using CDF, use edges, otherwise use centers
    if use_cdf is True:
        x = edges_pass
        y = edges_fail
    elif use_cdf is False:
    ...
    # (N - (B_p + B_f)) * ( epsilon * signal_pass + (1 - epsilon) * signal_fail) + B_p * bg_pass + B_f * bg_fail
    
    result_pass = (N - (B_p + B_f)) * epsilon * signal_pass + B_p * bg_pass
    #result_pass = (N) * epsilon * signal_pass + B_p * bg_pass
    result_fail = (N - (B_p + B_f)) * (1 - epsilon) * signal_fail + B_f * bg_fail
    #result_fail = (N) * (1 - epsilon) * signal_fail + B_f * bg_fail
    
    return result_pass, result_fail 

def calculate_custom_chi2(values, errors, model, n_params):
    mask = (errors > 0) & (model > 0)
    Pearson_chi2 = np.sum(((values[mask] - model[mask]) / errors[mask])**2)
    Poisson_chi2 = 2 * np.sum(model[mask] - values[mask] + values[mask] * np.log(values[mask]/model[mask]))
    ndof = mask.sum() - n_params
    return Pearson_chi2, Poisson_chi2, ndof

def fit_function(fit_type, hist_pass, hist_fail, fixed_params=None, use_cdf=False, x_min=x_min, x_max=x_max, interactive=False, args_bin=None):
    ...
    bounds.update({
        "N":   (0, N_p0, N_p0*4),
        "B_p": (0, B_p_p0/4, B_p_p0*3),
        "B_f": (0, B_f_p0, B_f_p0*3),
        "epsilon": (0.8, 0.95, 1)
    })
    ...
    if cov is None:
        ...
        print(f"Covariance matrix complete")
    ...
    print(f"Histogram Bin: \t{edges_pass[0]:.1f} - {edges_pass[-1]:.1f} GeV")
    print(f"Fit Success  : {'Yes' if m.valid else 'No'}")
    print("-" * 60)

    print("Fit Parameters:")
    print(f"  Total N         = {popt['N']:.1f} ± {perr['N']:.1f}")
    print(f"  Efficiency ε    = {popt['epsilon']:.6f} ± {perr['epsilon']:.6f}")
    print(f"  Background B_p  = {popt['B_p']:.1f} ± {perr['B_p']:.1f}")
    print(f"  Background B_f  = {popt['B_f']:.1f} ± {perr['B_f']:.1f}")
    print("-" * 60)

    print("Goodness-of-Fit:")
    print(f"  NLL χ² / ndf    = {chi2:.2f} / {dof} = {reduced_chi2:.3f}")
    print(f"  Pearson χ² / ndf= {Pearson_chi2:.2f} / {total_ndof} = {Pearson_tot_red_chi2:.3f}")
    print(f"  Poisson χ² / ndf= {Poisson_chi2:.2f} / {total_ndof} = {Poisson_tot_red_chi2:.3f}")
    print("-" * 60)

    print("Efficiency:")
    print(f"  ε = {popt['epsilon']:.4f} ± {perr['epsilon']:.4f} (bin {edges_pass[0]:.1f} - {edges_pass[-1]:.1f} GeV)")
...
def plot_combined_fit(results, plot_dir=".", data_type="DATA", fixed_params=None):
    ...
    #total_fail = (params["N"] - (params["B_p"] + params["B_f"])) * (1-params["epsilon"]) * signal_fail + bg_fail
    ...
    plt.plot(x, (params["N"] - (params["B_p"] + params["B_f"]))*(1-params["epsilon"])*signal_fail, 'r--', label="Signal")
...
BACKGROUND_MODELS = {
    "ps": {
        ...
        "cdf": None,  # could implement via betainc, but skip for now
        "params": ["a", "b"],
        "bounds": {
            "a": (0, 2, np.inf),
            "b": (0, 1, np.inf)
...
# NEW FILE PATH
    ...
    results = fit_function(args.type, hist_pass, hist_fail, fixed_params, use_cdf=args.cdf, interactive=args.interactive, args_bin=args.bin)
    if results is None:
