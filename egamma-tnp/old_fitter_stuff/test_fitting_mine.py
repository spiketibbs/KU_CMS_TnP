...
def double_crystal_ball(x, A, mu, sigma, alphaL, nL, alphaR, nR):
    def normalized_dcb(x, mu, sigma, alphaL, nL, alphaR, nR):
        def double_crystal_ball_unnormalized(x, mu, sigma, alphaL, nL, alphaR, nR):
            z = (x - mu) / sigma
            ...
            # Core
            mask_core = (z > -abs_alphaL) & (z < abs_alphaR)
            result[mask_core] = np.exp(-0.5 * z[mask_core]**2)

            # Left tail
            mask_left = z <= -abs_alphaL
            NL = (nL / abs_alphaL)**nL * np.exp(-0.5 * abs_alphaL**2)
            result[mask_left] = NL * (nL / abs_alphaL - abs_alphaL - z[mask_left])**(-nL)

            # Right tail
            mask_right = z >= abs_alphaR
            NR = (nR / abs_alphaR)**nR * np.exp(-0.5 * abs_alphaR**2)
            result[mask_right] = NR * (nR / abs_alphaR - abs_alphaR + z[mask_right])**(-nR)

            return result
        y_unnorm = double_crystal_ball_unnormalized(x, mu, sigma, alphaL, nL, alphaR, nR)
        norm = np.trapezoid(y_unnorm, x)
        return y_unnorm / norm
    return A * normalized_dcb(x, mu, sigma, alphaL, nL, alphaR, nR)

def double_voigtian(x, A, mu, sigma1, gamma1, sigma2, gamma2):
    def unormalized_double_voigtian(x, mu, sigma1, gamma1, sigma2, gamma2):
        voigt1 = voigt_profile(x - mu, sigma1, gamma1)
        voigt2 = voigt_profile(x - mu, sigma2, gamma2)
        return voigt1 + voigt2
    y_unnorm = unormalized_double_voigtian(x, mu, sigma1, gamma1, sigma2, gamma2)
    norm = np.trapezoid(y_unnorm, x)
    return A * (y_unnorm / norm)
...
def phase_space(x, B, a, b, x_min, x_max):
    def normalized_lin(x, a, b, x_min, x_max):
        def unnormalized_ps(x, a, b, x_min, x_max):
            safe_x = np.clip(x, x_min, x_max)
            shape = (safe_x - x_min) ** a * (x_max - safe_x) ** b
            shape[(x <= x_min) | (x >= x_max)] = 0
            return shape
        y_unnorm = unnormalized_ps(x, a, b, x_min, x_max)
        norm = np.trapezoid(y_unnorm, x)
        return y_unnorm / norm
    return B * normalized_lin(x, b, a, x_min, x_max)

def linear(x, B, b, C):
    def normalized_lin(x, b, C):
        def unnormalized_linear(x, b, C):
            shape = b + C * x
            return shape
        y_unnorm = unnormalized_linear(x, b, C)
        norm = np.trapezoid(y_unnorm, x)
        return y_unnorm / norm
    return B * normalized_lin(x, b, C)  

def exponential(x, B, C):
    def unnormalized_exp(x, C):
        shape = np.e ** (C * x)
        return shape
    y_unnorm = unnormalized_exp(x, C)
    norm = np.trapezoid(y_unnorm, x)
    return B * (y_unnorm / norm)
...
def dcb_plus_linear(x, A, mu, sigma, alphaL, nL, alphaR, nR, B, b, C):
    return double_crystal_ball(x, A, mu, sigma, alphaL, nL, alphaR, nR) + linear(x, B, b, C)
...
def dv_plus_linear(x, A, mu, sigma1, gamma1, sigma2, gamma2, B, b, C):
    return double_voigtian(x, A, mu, sigma1, gamma1, sigma2, gamma2) + linear(x, B, b, C)
...
def dg_plus_linear(x, A, mu, sigma_1, sigma_2, B, b, C):
    return double_gaussian(x, A, mu, sigma_1, sigma_2) + linear(x, B, b, C)
...
def compute_signal_background_events_dv_lin(x, popt):
    signal = double_voigtian(x, *popt[:6])
    background = linear(x, popt[6], popt[7], popt[8])
    return np.trapezoid(signal, x), np.trapezoid(background, x)
...
def compute_signal_background_events_dg_lin(x, popt):
    signal = double_gaussian(x, *popt[:4])
    background = linear(x, popt[4], popt[5], popt[6])
    return np.trapezoid(signal, x), np.trapezoid(background, x)
...
def perform_fit(type, hist, hist_name, fixed_params=None):
    ...
    FIT_CONFIGS = {
        ...
        "dcb_cheb": {
            ...
            "bounds": {
                "A": (max(values) * 0.1, max(values) * 2, np.inf),
                "mu": (89, 90, 91),
                "sigma": (2, 2.76, 2.77),
                "alphaL": (0.9, 0.94, 0.95),
                "nL": (0.1, 5, 30),
        ...
        "dg_lin": {
            "param_names": ["A", "mu", "sigma_1", "sigma_2", "B", "b", "C"],
            "bounds": {
                "A": (max(values) * 0.5, max(values) * 0.8, np.inf),
                "mu": (89, 90, 91),
    ...
    # DOUBLE CRYSTAL BALL PLUS LINEAR/EXPONENTIAL
        ...
        if type == "dcb_lin":
            ...
            def full_model(x, A, mu, sigma, alphaL, nL, alphaR, nR, B, b, C):
                return dcb_plus_linear(x, A, mu, sigma, alphaL, nL, alphaR, nR, B, b, C)
        elif type == "dcb_exp":
    ...
    # DOUBLE VOIGTIAN PLUS LINEAR/EXPONENTIAL
        ...
        if type == "dv_lin":
            ...
            def full_model(x, A, mu, sigma1, gamma1, sigma2, gamma2, B, b, C):
                return dv_plus_linear(x, A, mu, sigma1, gamma1, sigma2, gamma2, B, b, C)
        elif type == "dv_exp":
    ...
    # DOUBLE GAUSSIAN PLUS LINEAR/EXPONENTIAL
        ...
        if type == "dg_lin":
            ...
            def full_model(x, A, mu, sigma_1, sigma_2, B, b, C):
                return dg_plus_linear(x, A, mu, sigma_1, sigma_2, B, b, C)
        elif type == "dg_exp":
    ...
    popt, pcov, infodict, errmsg, ier = curve_fit(
        model, centers, values, p0=free_p0, sigma=errors, absolute_sigma=True, bounds=(free_lower, free_upper), full_output=True, maxfev=20000
    )
    ...
    if type == "dcb_ps":
        dB = full_perr[7]
        background_shape = (x - x_min) ** full_popt[8] * (x_max - x) ** full_popt[9]
        ...
        dB = full_perr[6]
        background_shape = (x - x_min) ** full_popt[7] * (x_max - x) ** full_popt[8]
        background_error = np.trapezoid(background_shape, x) * dB
    elif type == "dv_lin" or type == "dv_exp":
        background_error = np.sqrt(full_perr[6] ** 2)
    elif type == "dv_cheb":
        background_error = np.sqrt(np.sum(full_perr[6:] ** 2)) * (x_max - x_min)
    elif type == "dg_ps":
        dB = full_perr[4]
        background_shape = (x - x_min) ** full_popt[5] * (x_max - x) ** full_popt[6]
        background_error = np.trapezoid(background_shape, x) * dB
    elif type == "dg_lin" or type == "dg_exp":
        background_error = np.sqrt(full_perr[4] ** 2)
    elif type == "dg_cheb":
...
def plot_fit(fit_results, efficiency=None, efficiency_error=None, plot_dir=".", data_type="DATA", Npass=None, Nfail=None):
    ...
    plt.plot(x, combined, color="orange", label=signal_label)
    plt.plot(x, combined, color="red", label=background_label)
    ...
    plt.text(0.05, 0.95, legend_text, transform=plt.gca().transAxes, fontsize=9, verticalalignment="top", bbox=dict(facecolor="white", alpha=0.8))
    ...
    plt.savefig(f"{plot_dir}/{data_type}_{type}_fit_{hist_name}.png")
    plt.close()
...
def main():
    ...
    parser.add_argument("--bin", type=str, required=True, choices=bins.keys(), help="Which bin to fit (e.g., bin00, bin01, etc.)")
    parser.add_argument(
        "--type",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--fix", type=str, default="", help="Comma-separated list of parameters to fix FOR THE PASS FIT ONLY in format param1=value1,param2=value2"
    )
    ...
    if args.data == "DATA_barrel_1":
        ...
        root_file_path = "/uscms/home/hortua/nobackup/egamma-tnp/examples/nanoaod_filters_custom/blp_tagged/MC_DY_2023/get_1d_pt_eta_phi_tnp_histograms_1/MC_23D_histos_pt_barrel_1.root"
    ...
    params_to_float_in_fail = ["A", "B"]  # Signal amplitude 'A' always floats
