...
def create_combined_model(fit_type):
    ...
    def model(xe, *params):
        n_pass = len(xe) // 2
        edges_pass = xe[:n_pass]    
       # print(f"Edges pass: {len(edges_pass)}")
        edges_fail = xe[n_pass:]
        #print(f"Edges fail: {len(edges_fail)}")
     #   n_pass = len(centers) // 2
     #   centers_pass = centers[:n_pass]   # Pass region centers
     #   centers_fail = centers[n_pass:]   # Fail region centers

        P = dict(zip(param_names, params))
        # Split combined edges into pass/fail
        ...
        model_pass = N * epsilon * signal_pass + bg_pass
        model_fail = N * (1 - epsilon) * signal_fail + bg_fail
...
class PassFailPlotter:
    ...
    def __call__(self, args):
        ...
        signal_y_pass = N * epsilon * signal_pass
        signal_y_fail = N * (1 - epsilon) * signal_fail
...
def fit_combined(fit_type, hist_pass, hist_fail, fixed_params=None, x_min=x_min, x_max=x_max, interactive=False):
    ...
    print(f"Length of centers pass: {len(centers_pass)}")
    print(f"Centers pass: {centers_pass}")
    ...
    print(f"Initial N value: {N_p0*0.3}")
    ...
    for name in param_names:
        ...
        if name == "N":
            ...
            initial_guesses[name] = initial_epsilon
        else:
    ...
    print(f"Length of Values pass: {len(values_pass)}")
    print(f"Length of Values fail: {len(values_fail)}")
    print(f"Length of Centers pass: {len(centers_pass)}")
    print(f"Length of Centers fail: {len(centers_fail)}")
    print(f"Length of Edges pass: {len(edges_pass)}")
    print(f"Edges pass: {(edges_pass)}")
    print(f"Length of Edges fail: {len(edges_fail)}")
    print(f"Edges fail: {(edges_fail)}")
    ...
    combined_edges = np.concatenate([edges_pass, edges_pass[1:]])
    print(f"Length of Combined Edges: {len(combined_edges)}")

    # Create model and cost function
    n_bins_pass = len(edges_pass) - 1
    ...
    # Base params:
    base_names = ["N", "epsilon", "B_p", "B_f"]
    # Signal params from config:
    signal_params = SIGNAL_MODELS[fit_type.split('_')[0]]["params"]
    # Background params original names:
    bg_params = BACKGROUND_MODELS[fit_type.split('_')[1]]["params"]  # e.g., ["b", "C"]
    # Make unique pass/fail names:
    bg_params_pass = [f"{p}_pass" for p in bg_params]
    bg_params_fail = [f"{p}_fail" for p in bg_params]

    combined_param_names = base_names + signal_params + bg_params_pass + bg_params_fail
    ...
    print(f"Length of edges_pass: {len(edges_pass)}")
    print(f"Length of edges_fail: {len(edges_fail)}")
...
def plot_combined_fit(results, plot_dir=".", data_type="DATA", fixed_params=None):
    ...
    epsilon_err = results["perr"]["epsilon"]
    ...
    signal_pass = params["N"] * epsilon * signal_func(x, *signal_params)
    ...
    info_text = [
        ...
        f"ε_raw = {params['epsilon']:.6f} ± {results['perr']['epsilon']:.6f}",
        f"ε = {epsilon:.6f} ± {epsilon_err:.6f}",
        f"B_p = {params['B_p']:.1f} ± {results['perr']['B_p']:.1f}",
        f"B_f = {params['B_f']:.1f} ± {results['perr']['B_f']:.1f}",
        f""
        f"Signal yield: {params['N']*epsilon:.1f}",
    ...
    total_fail = params["N"] * (1-epsilon) * signal_fail + bg_fail
    ...
    plt.plot(x, params["N"]*(1-epsilon)*signal_fail, 'r--', label="Signal")
    ...
    info_text = [
        ...
        f"B_f = {params['B_f']:.1f} ± {results['perr']['B_f']:.1f}",
        f""
        f"Signal yield: {params['N']*(1-epsilon):.1f}",
...
def main():
    parser = argparse.ArgumentParser(description="Fit ROOT histograms with combined pass/fail model.")
    ...
    parser.add_argument("--data", required=True, choices=["DATA_barrel_1_tag", ...])  # Keep existing choices
...
# NEW FILE PATH
    ...
    results = fit_combined(args.type, hist_pass, hist_fail, fixed_params, interactive=args.interactive)
    ...
    epsilon_err = results["perr"]["epsilon"]
    print(f"Efficiency: {epsilon:.6f} ± {epsilon_err:.6f}")
