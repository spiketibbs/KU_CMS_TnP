...
def main():
    ...
    if args.executor in ["multiprocessing", "processes", "single-threaded", "sync", "synchronous", "threading", "threads"]:
        ...
        # Prefer the container you are already running in; fallback to env var or a default.
        singularity_image = os.environ.get(
            "SINGULARITY_CONTAINER",
            "/cvmfs/unpacked.cern.ch/.flat/ec/ec725d4305197677910b259e7b74e7bb4478a9fa6263a744ba3ae7af5b27906b",
        )

        # The job_script_prologue runs before the worker process starts on the compute node.
        # We export the proxy and ensure PYTHONPATH points at the working directory (condor scratch).
        job_prologue = [
            # Ensure apptainer or singularity is in PATH
            "if [ -f /cvmfs/oasis.opensciencegrid.org/mis/apptainer/current/bin/apptainer ]; then "
            "  export PATH=/cvmfs/oasis.opensciencegrid.org/mis/apptainer/current/bin:$PATH; "
            "elif [ -f /cvmfs/grid.cern.ch/utilities/bin/apptainer ]; then "
            "  export PATH=/cvmfs/grid.cern.ch/utilities/bin:$PATH; "
            "else "
            "  echo '[WARN] No apptainer found in known CVMFS locations' >&2; "
            "fi",
        ...
        # If your container already has the Python environment installed, we don't need to ship /srv/.env.
        # Set ship_env=False so the workers will use the image's environment.
        cluster = LPCCondorCluster(
            ship_env=True,  # False because the singularity image already contains the env
            #image=singularity_image,  # use the same image your interactive shell is using
            transfer_input_files=[_x509_path],  # copy proxy into the worker working dir
            ...
            # Optional: you can add +JobFlavour or other condor directives via job_extra if needed:
            # job_extra={"JobFlavour": f'"{args.jobflavour}"'},
        )
