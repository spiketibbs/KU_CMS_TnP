...
def main():
    ...
    if args.executor in ["multiprocessing", "processes", "single-threaded", "sync", "synchronous", "threading", "threads"]:
        ...
        logger.info("Running with LPCCondorCluster using your local myenv environment")
        ...
        cluster = LPCCondorCluster(
            ...
            job_script_prologue=[
                "echo "=== ENV DEBUG START ===" >&2",
                "echo "whoami: $(whoami)" >&2",
                "echo "PATH: $PATH" >&2",
                "echo "which python: $(which python || true)" >&2",
                "python - <<'PY'",
                "import sys, pkgutil, shutil, importlib",
                "print("sys.executable", sys.executable)",
                "print("sys.version", sys.version)",
                "print("sys.path:", sys.path[:5])",
                "print("git present:", bool(shutil.which("git")))",
                "for p in ("pip","dask","distributed","uproot","lpcjobqueue","coffea","dask_awkward"):",
                "    try:",
                "        m = importlib.import_module(p)",
                "        print(p, "version:", getattr(m,"__version__","(no __version__)"))",
                "    except Exception as e:",
                "        print(p, "IMPORT_FAIL:", e)",
                "PY",
                "echo "Which ps: $(which ps || true)" >&2",
                "echo "Can curl github?: $(curl -sSf https://raw.githubusercontent.com/ --max-time 10 >/dev/null && echo yes || echo no)" >&2",
                "echo "=== ENV DEBUG END ===" >&2",
                "source /cvmfs/cms.cern.ch/cmsset_default.sh",
                "export XRD_RUNFORKHANDLER=1",
                f"export X509_USER_PROXY={get_proxy()}",
                "echo 'Worker started in environment:'",
                "which python",
                "echo 'Installing lpcjobqueue on worker...'",
                "python -m pip install --user lpcjobqueue",  # <-- ADD THIS LINE
                "python -c 'import lpcjobqueue, sys; print(f\"[Worker OK] lpcjobqueue={lpcjobqueue.__version__}\")'",
            ],
        )
